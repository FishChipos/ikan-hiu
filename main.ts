...
function turn_left() {
    ...
    while (true) {
        ...
        sensors.DDMmotor(gb1_direction, 1, gb1_speed, turn_speed)
        sensors.DDMmotor(gb2_direction, 1, gb2_speed, turn_speed)
...
function turn_right() {
    ...
    while (true) {
        basic.showNumber(counter)
        sensors.DDMmotor(gb1_direction, 0, gb1_speed, turn_speed)
        sensors.DDMmotor(gb2_direction, 0, gb2_speed, turn_speed)
...
function forward(time: number) {
    sensors.DDMmotor(gb1_direction, 0, gb1_speed, speed)
    sensors.DDMmotor(gb2_direction, 1, gb2_speed, speed)
    basic.pause(time)
...
function back(time: number) {
    sensors.DDMmotor(gb1_direction, 1, gb1_speed, speed)
    sensors.DDMmotor(gb2_direction, 0, gb2_speed, speed)
    basic.pause(time)
    //  Car is angled too far to the right
    if (pins.digitalReadPin(ir1) == 1 && pins.digitalReadPin(ir2) == 0) {
        stop()
